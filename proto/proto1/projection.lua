...
outside of screen. To create projection of 1x1 tile, we do:
   ...
   1) apply scale to tile
   2) rotate by beta around Y
   3) rotate by alpha around X
   4) project to screen place (Z=0)
...
coordinates by:
  ...
  y' = -scale*(sin(alpha)*sin(beta)*x + cos(alpha)*y - sin(alpha)*cos(beta)*z)
...
v3' = (0, -scale*cos(alpha))

which is equal to

v3' = (0, -10*math.sqrt(15))

which is approx

v3''= (0, -38.72983346207417)

meaning, that 39 pixels are approximately 1 meter when we take height into
account.
...
local ca = 10*math.sqrt(15) -- scale*math.cos(alpha)
...
projection.SPAN_V3_x = 0
projection.SPAN_V3_y = ca
